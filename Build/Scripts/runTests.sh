...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="web/typo3conf/ext/oauth2_server/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="web/typo3conf/ext/oauth2_server/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="web/typo3conf/ext/oauth2_server/Tests/Functional"
            ;;
        unit)
            TEST_FILE="web/typo3conf/ext/oauth2_server/Tests/Unit"
            ;;
    esac
fi
